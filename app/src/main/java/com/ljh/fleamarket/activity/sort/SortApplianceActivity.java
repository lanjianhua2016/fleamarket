...
public class SortApplianceActivity extends AppCompatActivity {
    private Button backtosort2;
    ...
    void setInit(){
        ...
        DisplayGoods();
        RefreshLayout = (RefreshLayout) findViewById(R.id.smartrefresh_layout_sort_appliance);
...
//        recyclerView.addItemDecoration(dividerItemDecoration);
                        ...
                        public void run() {
                            ...
                            if (responseBO.flag == 200) {
                                ...
                                if (resultGoodsList.isEmpty()){
                                    Toast.makeText(SortApplianceActivity.this, "暂无数据!", Toast.LENGTH_SHORT).show();
                                }else {
                                    //创建适配器
                                    recyclerAdapter = new MyGoodsRecyclerAdapter(SortApplianceActivity.this, resultGoodsList);
                                    //视图加载适配器
                                    recyclerView.setAdapter(recyclerAdapter);//设置Adapter(适配器)
                                    Toast.makeText(SortApplianceActivity.this, "刷新成功!", Toast.LENGTH_SHORT).show();
                                }
                            }
                            else if(responseBO.token==null){
                                Toast.makeText(SortApplianceActivity.this, "身份验证过期,请重新登录!", Toast.LENGTH_SHORT).show();
                            }else {
                        ...
                        public void run() {
                            if (responseBO.flag == 200) {
                                ...
                                if (loadMoreGoodsList.isEmpty()){
                                    Toast.makeText(SortApplianceActivity.this, "没有更多了!!", Toast.LENGTH_SHORT).show();
                                }else {
                                    for(int i=0;i<loadMoreGoodsList.size();i++){
                                        resultGoodsList.add(loadMoreGoodsList.get(i));
                                    }
                                    recyclerAdapter.notifyDataSetChanged();
                                    Toast.makeText(SortApplianceActivity.this, "加载成功!", Toast.LENGTH_SHORT).show();
                                    //pullToRefreshAndPushToLoadView.finishLoading();
                                }
    ...
    public void ConnetctFailed(){
        if (isNetConnected(SortApplianceActivity.this)==false){
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(SortApplianceActivity.this, "网络不给力哦!", Toast.LENGTH_SHORT).show();
                    pd.dismiss();
                }
            });
        }else {
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(SortApplianceActivity.this, "服务器出错啦，请稍后再试!", Toast.LENGTH_SHORT).show();
                    pd.dismiss();
                }
            });
        }
