...
public class SortClothesActivity extends AppCompatActivity {
    private Button backtosort5;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        backtosort5 = (Button)findViewById(R.id.backto_sort5);
        backtosort5.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (dataApp.isIntentPermission()) {
                    finish();
                } else {
                    Toast.makeText(SortClothesActivity.this, "请稍等...", Toast.LENGTH_SHORT).show();
                }
    ...
    void setInit(){
        ...
        DisplayGoods();
        RefreshLayout = (RefreshLayout) findViewById(R.id.smartrefresh_layout_sort_clothes);
...
//        recyclerView.addItemDecoration(dividerItemDecoration);
                        ...
                        public void run() {
                            ...
                            if (responseBO.flag == 200) {
                                ...
                                if (resultGoodsList.isEmpty()){
                                    Toast.makeText(SortClothesActivity.this, "暂无数据!", Toast.LENGTH_SHORT).show();
                                }else {
                                    //创建适配器
                                    recyclerAdapter = new MyGoodsRecyclerAdapter(SortClothesActivity.this, resultGoodsList);
                                    //视图加载适配器
                                    recyclerView.setAdapter(recyclerAdapter);//设置Adapter(适配器)
                                    Toast.makeText(SortClothesActivity.this, "刷新成功!", Toast.LENGTH_SHORT).show();
                                }
                            }
                            else if(responseBO.token==null){
                                Toast.makeText(SortClothesActivity.this, "身份验证过期,请重新登录!", Toast.LENGTH_SHORT).show();
                            }else {
                                Toast.makeText(SortClothesActivity.this, "刷新失败!", Toast.LENGTH_SHORT).show();
                            }
                            dataApp.setIntentPermission(true);
                        }
                    });
                        ...
                        public void run() {
                            if (responseBO.flag == 200) {
                                ...
                                if (loadMoreGoodsList.isEmpty()){
                                    Toast.makeText(SortClothesActivity.this, "没有更多了!!", Toast.LENGTH_SHORT).show();
                                }else {
                                    for(int i=0;i<loadMoreGoodsList.size();i++){
                                        resultGoodsList.add(loadMoreGoodsList.get(i));
                                    }
                                    recyclerAdapter.notifyDataSetChanged();
                                    Toast.makeText(SortClothesActivity.this, "加载成功!", Toast.LENGTH_SHORT).show();
                                    //pullToRefreshAndPushToLoadView.finishLoading();
                                }
                                ...
                                Toast.makeText(SortClothesActivity.this, "加载失败!", Toast.LENGTH_SHORT).show();
                            }
                            dataApp.setIntentPermission(true);
                        }
                    });
    ...
    public void ConnetctFailed(){
        if (isNetConnected(SortClothesActivity.this)==false){
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(SortClothesActivity.this, "网络不给力哦!", Toast.LENGTH_SHORT).show();
                    pd.dismiss();
                }
            });
        }else {
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(SortClothesActivity.this, "服务器出错啦，请稍后再试!", Toast.LENGTH_SHORT).show();
                    pd.dismiss();
                }
            });
        }
